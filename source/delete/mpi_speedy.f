...
! This subroutine initializes all mpi variables
      ...
      complex*16 CA_Z(0:NX_S,0:NZ,0:NY_S)
      ...
      BLOCK=DIAG + shift*dir*(1-2*MOD(DIAG/NP_S,2))
...
! Get a block from the source array to be sent to another process
        DO I=0,NKX_S
          DO K=0,NZ_S
            DO J=0,NY_S
              SENDBUF(I,K,J)=CA_Z(I,K+(NZ_S+1)*BLOCK,J)
            END DO
          END DO
        END DO
...
! Place the newly recieved data into a temporary storage array
        DO I=0,NKX_S
          DO K=0,NZ_S
            DO J=0,NY_S
              IF (I+(NKX_S+1)*BLOCK.le.NX) THEN
! Make sure that we don't write beyond the end of the array
                B(I+(NKX_S+1)*BLOCK,K,J)=RECVBUF(I,K,J)
              END IF
            END DO
          END DO
        END DO
...
! Here block=diag, we just need to copy locally from A->B
          ...
          DO I=0,NKX_S
            DO K=0,NZ_S
              DO J=0,NY_S
              IF (I+(NKX_S+1)*BLOCK.le.NX) THEN
                B(I+(NKX_S+1)*BLOCK,K,J)=sendbuf(I,K,J)
              END IF
              END DO
            END DO
          END DO
...
! This subroutine initializes all mpi variables
      ...
      BLOCK=DIAG + shift*dir*(1-2*MOD(DIAG,2))
...
! Get a block from the source array to be sent to another process
        DO I=0,NKX_S
          DO K=0,NZ_S
            DO J=0,NY_S
              SENDBUF(I,K,J)=CA_X(I+(NKX_S+1)*BLOCK,K,J)
            END DO
          END DO
        END DO
...
! This subroutine initializes all mpi variables
      ...
      complex*16 CA_Z(0:NX_S,0:NZ,0:NY_S)
      complex*16 CA_Y(0:NX_S,0:NZ_S,0:NY)
      complex*16 B(0:NX_S,0:NZ,0:NY_S)
...
! Get a block from the source array to be sent to another process
        DO I=0,NKX_S
          DO K=0,TNKZ_S
            DO J=0,NY_S
              SENDBUF(I,K,J)=CA_Y(I,K,J+(NY_S+1)*BLOCK)
            END DO
          END DO
        END DO
...
! Place the newly recieved data into a temporary storage array
        DO I=0,NKX_S
          DO K=0,TNKZ_S
            DO J=0,NY_S
              IF (K+(TNKZ_S+1)*BLOCK.le.NZ) THEN
! Make sure that we don't write beyond the end of the array
...
! Get a block from the source array to be sent to another process
        DO I=0,NKX_S
          DO K=0,TNKZ_S
            DO J=0,NY_S
              SENDBUF(I,K,J)=CA_Z(I,K+(TNKZ_S+1)*BLOCK,J)
            END DO
          END DO
        END DO
