...
C Input array is in Fourier space and local in X
      COMPLEX*16 CU(0:NX_S,0:NZ_S,0:NY+1)
...
C Intermediate arrays local X, and Z, These are all equivalenced
      ...
      COMPLEX*16 CU_Y(0:NX_S,0:NZ_S,0:NY+1)
      COMPLEX*16 CU_Z(0:NX_S,0:NZ+1,0:NY_S)
...
C Perform a parallel transpose to get data stored locally in the z-direction
      CALL TRANSPOSE_MPI_Y_TO_Z(CU_Y,CU_Z,NX+1,NY+1,NZ+1,NX_S,NY_S,NZ_S)
      CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
C Perform a parallel transpose to get data stored locally in the y-direction
      CALL TRANSPOSE_MPI_Z_TO_X(CU_Z,CU_X,NX+1,NY+1,NZ+1,NX_S,NY_S,NZ_S)
      CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
C Set data above NXM=0
      ...
      END DO
      DO J=NY-INT(RANK/NP_S)*(NY_S+1),NY_S
...
C Parallel transpose to get data stored locally in the z-direction
      CALL TRANSPOSE_MPI_X_TO_Z(CU_X,CU_Z,NX+1,NY+1,NZ+1,NX_S,NY_S,NZ_S)
      CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
C Parallel transpose to get data stored locally in the z-direction
      CALL TRANSPOSE_MPI_Z_TO_Y(CU_Z,CU_Y,NX+1,NY+1,NZ+1,NX_S,NY_S,NZ_S)
      CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      ...
      INCLUDE 'header_mpi'
      INTEGER  I, J, K
      COMPLEX*16   CU(0:NX_S,0:NZ+1,0:NY_S)
      COMPLEX*16   U (0:NX_S,0:NZ+1,0:NY_S)

      COMPLEX*16 CZX_PLANE(0:NZ,0:NX_S)
      ...
      DO J=0,NY_S
        DO I=0,NX_S
         DO K=0,NKZ
          ...
          END DO
        END DO
        CALL FFTWND_F77(FFTW_Z_TO_P_PLAN, NX_S+1,
...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      ...
      INTEGER  I, J, K
      COMPLEX*16   CU(0:NX_S,0:NZ_S,0:NY+1)
      COMPLEX*16   U (0:NX_S,0:NZ_S,0:NY+1)
