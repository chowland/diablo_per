...
! Skip the rest of this j index
      ...
      INTEGER X_BLOCK
      INTEGER Z_BLOCK
      INTEGER BLOCK_LEN_X(3)
      INTEGER DISPLACEMENT_X(3)
      INTEGER TYPES_X(3) 
      DATA TYPES_X/MPI_LB,MPI_DOUBLE_COMPLEX,MPI_LB/

      INTEGER HEADER

! For debugging
      CHARACTER*(MPI_MAX_PROCESSOR_NAME) hostname
      REAL*8 CHECK_START_TIME,CHECK_END_TIME
      INTEGER len  
...
!        offset=4*3
!        CALL MPI_FILE_SEEK(fh,OFFSET,MPI_SEEK_SET,IERROR)
! Offset=5*4-byte integers + 1*8-byte double precision
!        offset=5*4+8
!        CALL MPI_FILE_SEEK(fh,OFFSET,MPI_SEEK_CUR,IERROR)
!      END IF
...
! Write the Fortran pre-header containing the number of bytes
       ...
       write(100+RANK,*) 'RANK,NUM_WRITE_X: ',RANK,NUM_WRITE_X

! Create the MPI FILE TYPES
      PRE_OFFSET_X=(MOD(RANK,NP_S)*(NKX_S+1))*16
      POST_OFFSET_X=((NKX+1)-NUM_WRITE_X-(NKX_S+1)*MOD(RANK,NP_S))*16
      BLOCK_LEN_X(1)=1
      BLOCK_LEN_X(2)=NUM_WRITE_X
      BLOCK_LEN_X(3)=1
      DISPLACEMENT_X(1)=0
      DISPLACEMENT_X(2)=PRE_OFFSET_X
      DISPLACEMENT_X(3)=NUM_WRITE_X*16+POST_OFFSET_X

      CALL MPI_TYPE_STRUCT(3,BLOCK_LEN_X,DISPLACEMENT_X,TYPES_X
     &                      ,X_BLOCK,IERR)
      CALL MPI_TYPE_COMMIT(X_BLOCK,IERR)

      CALL MPI_FILE_SET_VIEW(fh,0,MPI_DOUBLE_COMPLEX,X_BLOCK,'native',
     &              MPI_INFO_NULL,IERR)
...
!        do while ((k.le.TNKZ_S)
!     &           .AND.((RANK/NP_S)*(TNKZ_S+1)+K.le.TNKZ))
          write(200+RANK,*) 'RANK,NUM_WRITE_X: ',RANK,NUM_WRITE_X
          write(200+RANK,*) 'IERR: ',IERR
          write(200+RANK,*) (CU1(i,k,j),i=0,NUM_WRITE_X-1) 
...
! New method:
     ...
     &                       ,MPI_DOUBLE_COMPLEX,status,IERR)
          write(300+RANK,*) 'RANK,NUM_WRITE_X: ',RANK,NUM_WRITE_X
          write(300+RANK,*) 'IERR: ',IERR
          write(300+RANK,*) (CU1(i,k,j),i=0,NUM_WRITE_X-1) 
...
! DEBUGGING GET RUNTIME
      IF (RANK.eq.0) THEN
      CALL DATE_AND_TIME (VALUES=TIME_ARRAY)
      CHECK_START_TIME=TIME_ARRAY(5)*3600+TIME_ARRAY(6)*60
...
! Write the Fortran post-header containing the number of bytes in this line
     ...
     &                      ,MPI_INTEGER,status,IERROR)
      ELSE
! ...or skip the header
! Offset=3*4-byte headers + 5*4-byte integers + 1*8-byte double precision
        offset=3*4+5*4+8
        CALL MPI_FILE_SEEK(fh_th(n),OFFSET,MPI_SEEK_SET,IERROR)
      END IF
...
! Write the Fortran post-header
       IF (RANK.eq.0) THEN
         ...
         CALL MPI_FILE_WRITE(fh_th(n),HEADER,1
     &                       ,MPI_INTEGER,status,IERROR)
! DEBUGGING WRITE RUNTIME
      CALL DATE_AND_TIME (VALUES=TIME_ARRAY)
      CHECK_END_TIME=TIME_ARRAY(5)*3600+TIME_ARRAY(6)*60
     &              +TIME_ARRAY(7)+TIME_ARRAY(8)*0.001
      CALL MPI_GET_PROCESSOR_NAME(hostname,len,IERROR)
      write(*,*) 'write time: ',RANK,hostname
