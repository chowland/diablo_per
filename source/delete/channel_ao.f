...
! Linear profile
              DO J=0,NY+1
! For one layer
!                TH(I,K,J,N)=-0.000917d0*(GYF(J)-GYF(1))
! For two layers
                if (J.lt.66) then
                TH(I,K,J,N)=-0.00917*(GYF(J)-GYF(1))
                else
                TH(I,K,J,N)=TH(I,K,65,N)-0.000917*(GYF(J)-GYF(65))
                end if
              END DO
         END DO
        END DO
...
! Here, In the 2d case we want to add a kick to the mean in z
     ...
     &             +(RNUM1-0.5)*KICK*EXP(-(GYF(J)*20.d0)**2.d0)
      ...
      write(*,*) 'calling ghost cell'

      IF (USE_MPI) THEN
        CALL GHOST_CHAN_MPI
      END IF

      write(*,*) 'done ghost cell'
...
C Check to make sure that grid file is the correct dimensions
         ...
         DO J=1,NY+1
           ...
           IF (VERBOSITY .GT. 3) WRITE(6,*) 'FNAME, GY(',J,') = '
     &           ,FNAME,GY(J)
         END DO
         DO J=1,NY
...
! Write out one plane
      if (MOVIE) then
        open(92,file='movie_q_x.txt',status='unknown',form='formatted')
        do i=0,NXM
        do J=1,NY
          write(92,*) S1(I,0,J)
        end do
...
! Calculate the viscous PV fluxes
     ...
     &                                /DYF(j)
      ...
      if (MOVIE) then
        if (N_TH.gt.0) then
        open(79,file='movie_th.txt',status='unknown',form='formatted')
        do I=0,NXM
        do J=1,NY
          write(79,*) TH(I,0,J,1)
        end do
...
! This file will contain a time history over a plane
        open(75,file='movie_u.txt',status='unknown',form='formatted')
        do I=0,NXM
        do J=1,NY
          write(75,*) U1(I,0,J)
        end do
        ...
        open(77,file='movie_v.txt',status='unknown',form='formatted')
        do I=0,NXM
        do J=1,NY
          write(77,*) U2(I,0,J)
        end do
        ...
        open(78,file='movie_w.txt',status='unknown',form='formatted')
        do I=0,NXM
        do J=1,NY
          write(78,*) U3(I,0,J)
        end do
...
! Set DELTA_T to some small default value
        ...
        write(*,*) 'WARNING: DELTA_T > 1, value capped at 1'
        DELTA_T=1.d0
      else
        DELTA_T=dt
