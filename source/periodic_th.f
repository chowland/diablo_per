...
C  the pressure gradient treated with Explicit Euler:
            TEMP5=1-TEMP1*(KX2_S(I)+1.d-4*KY2(J)+KZ2_S(K))**BETA
...
C diffusive term for each scalar
     ...
     &               +1.d-4*KY2_TH(J)+KZ2_S_TH(K))**BETA
     &               -REACTION(N)*TEMP1
...
C Note that we can use F1 and F2 as temporary storage variables here
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CSTH1(I,K,J)=CIKX_S(I)*CTH(I,K,J,1)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
      ...
      DO J=0,NY_S_TH
        DO K=0,NZ_S_TH
          DO I=0,NXM_TH
            STH2(I,K,J)=STH2(I,K,j)+STH1(I,K,J)**2.d0
          END DO
        END DO
      END DO
      DO J=0,TNKY_TH
...
! Note, here the constant k depends on the particular value of CHI
      DO J=0,NY_S_TH
        DO K=0,NZ_S_TH
          DO I=0,NXM_TH
            STH2(I,K,J)=STH2(I,K,j)+STH1(I,K,J)**2.d0
            STH2(I,K,J)=sqrt(STH2(I,K,J))
            STH1(I,K,J)=V_S(2)
     &            *TANH(V_S(2)*STH2(I,K,J)/CHI(2))/STH2(I,K,J)
          END DO
        END DO
      END DO
...
C First, CHI*d/dx(B*dC/dx):
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CSTH2(I,K,J)=CIKX_S(I)*CTH(I,K,J,1)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
        CALL FFT_XZY_MPI_TO_PHYSICAL_TH(CSTH2,STH2)
      ELSE
        CALL FFT_XZY_TO_PHYSICAL_TH(CSTH2,STH2)
      END IF
      DO J=0,NY_S_TH
        DO K=0,NZ_S_TH
          DO I=0,NXM_TH
            STH2(I,K,J)=STH2(I,K,J)*TH(I,K,J,N)*STH1(I,K,J)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
        CALL FFT_XZY_MPI_TO_FOURIER_TH(STH2,CSTH2)
      ELSE
        CALL FFT_XZY_TO_FOURIER_TH(STH2,CSTH2)
      END IF
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CFTH(I,K,J,N)=CFTH(I,K,J,N)-CIKX_S_TH(I)*CSTH2(I,K,J)
          END DO
        END DO
      END DO
...
C Now, calculate the CHI*d/dy(B*dC/dy) term:
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CSTH2(I,K,J)=CIKY_TH(J)*CTH(I,K,J,1)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
      ...
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CFTH(I,K,J,N)=CFTH(I,K,J,N)-CIKY_TH(J)*CSTH2(I,K,J)
          END DO
        END DO
      END DO
C Finally, calculate the CHI*d/dz(B*dC/dz) term:
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CSTH2(I,K,J)=CIKZ_S_TH(K)*CTH(I,K,J,1)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
      ...
      DO J=0,TNKY_TH
        DO K=0,TNKZ_S_TH
          DO I=0,NKX_S_TH
            CFTH(I,K,J,N)=CFTH(I,K,J,N)-CIKZ_S_TH(K)*CSTH2(I,K,J)
          END DO
        END DO
      END DO
C End if CHI(N).NE.0
...
C advection terms
      ...
      IF (N_TH.gt.0) THEN
        IF (USE_MPI) THEN
          CALL FFT_XZY_MPI_TO_PHYSICAL_TH(CTH(0,0,0,1),TH(0,0,0,1))
        ELSE
          CALL FFT_XZY_TO_PHYSICAL_TH(CTH(0,0,0,1),TH(0,0,0,1))
        END IF
      END IF

C Compute the nonlinear terms for the passive scalar equation
C Use Fourier interpolation to get velocity on scalar grid

      CSTH1(:,:,:)=0.d0
      DO J=0,NKY
        DO K=0,NKZ_S
          DO I=0,NKX_S
            CSTH1(i,k,j)=CU1(i,k,j)
          END DO
        END DO
      END DO
      DO J=1,NKY
        DO K=1,NKZ_S
          DO I=0,NKX_S
            CSTH1(i,TNKZ_S_TH+1-k,TNKY_TH+1-j)=CU1(i,k,j)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
...
C Use Fourier interpolation to get velocity on scalar grid
      ...
      DO J=1,NKY
        DO K=1,NKZ_S
          DO I=0,NKX_S
            CSTH1(i,TNKZ_S_TH+1-k,TNKY_TH+1-j)=CU2(i,k,j)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
...
C Use Fourier interpolation to get velocity on scalar grid
      ...
      DO J=1,NKY
        DO K=1,NKZ_S
          DO I=0,NKX_S
            CSTH1(i,TNKZ_S_TH+1-k,TNKY_TH+1-j)=CU3(i,k,j)
          END DO
        END DO
      END DO
      IF (USE_MPI) THEN
...
C (In the fully-periodic case, this is easy!)
      DO J=0,TNKY
        DO K=0,TNKZ_S
          DO I=0,NKX_S
            TEMP5=1.d0+TEMP1*(KX2_S(I)+1.d-4*KY2(J)+KZ2_S(K))**BETA
...
!     &                 -((GZ_S(K)-0.402)**2.d0/(2.d0*(lp/2.d0)**2.d0)) )
              ELSE
! Uniform initial bacterial distribution
        TH(I,K,J,N)=1.d0
...
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      ...
      INTEGER I,J,K
         ...
         DO K=0,NZ_TH
           GZ_TH(K)=(K*LZ)/NZ_TH
           DZ_TH(K)=LZ/NZ_TH
           IF (VERBOSITY .GT. 3) WRITE(6,*) 'GZ(',K,') = ',GZ_TH(K)
         END DO
         IF (FLAVOR.NE.'Ensemble') WRITE (6,*) 'Fourier in Y'
         DO J=0,NY_TH
           GY_TH(J)=(J*LY)/NY_TH
...
C Set the filtering constants for the all directions
      ...
      DO i=0,NKX_S_TH
       DO k=0,TNKZ_S_TH
        DO j=0,TNKY_TH
          sigma0=0.5d0*(1.d0+
     &       cos(sqrt((KX_TH(i)*LX*1.d0/float(NX_TH))**2.d0
     &            +(KZ_TH(k)*LZ*1.d0/float(NZ_TH))**2.d0
