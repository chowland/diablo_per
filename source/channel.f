...
! Now compute the implicit viscous terms 
! Compile terms of Ai in CFi which will be saved for next time step
      DO J=JSTART_TH(N),JEND_TH(N)
        DO K=0,TNKZ
          DO I=0,NKX
            CRTH(I,K,J,N)=CRTH(I,K,J,N)
     &                   -(TEMP1/PR(N)) *(KX2(I)+KZ2(K)) * CTH(I,K,J,N)
          END DO
        END DO
      END DO
      DO J=JSTART_TH(N),JEND_TH(N)
...
! Add the term owing the horizontal density gradient
      ...
      IF ((N.eq.1).AND.(I_RO_TAU.ne.0.d0)) then
      DO J=JSTART_TH(N),JEND_TH(N)
        DO K=0,TNKZ
          DO I=0,NKX
...
! Ui and TH should now be stored in z-lines
...
C Start with the diagonal terms (e.g. d/dx(U1^2))
C U1*U1
      DO J=JSTART,JEND
        DO K=0,NZM
          DO I=0,NXM
...
! U1*U2
! Note, these terms need to be treated separately since the interpolation
! formula are different for the F1 and F2 terms
      DO J=JSTART,JEND+1
        DO K=0,NZM
          DO I=0,NXM
            S1(I,K,J)=((U1(I,K,J)+U1(I,K,J-1))/2.d0)
     &                  *U2(I,K,J) 
          END DO
        END DO
      END DO
      ...
      DO J=JSTART,JEND
        DO K=0,TNKZ
          DO I=0,NKX
            CF1(I,K,J)=CF1(I,K,J) - (CS1(I,K,J+1)-CS1(I,K,J))/DYF(J)
         END DO
        END DO
      END DO
...
! U3*U2
      DO J=JSTART+1,JEND
        DO K=0,NZM
          DO I=0,NXM
            S1(I,K,J)=((U3(I,K,J)+U3(I,K,J-1))/2.d0)
     &                  *U2(I,K,J)
          END DO
        END DO
      END DO
      ...
      DO J=JSTART,JEND
        DO K=0,TNKZ
          DO I=0,NKX
            CF3(I,K,J)=CF3(I,K,J) - (CS1(I,K,J+1)-CS1(I,K,J))/DYF(J)
         END DO
        END DO
      END DO
...
C -- Done computing the nonlinear terms --
...
! U3*TH 
      ...
      END DO
! U2*TH
      DO J=JSTART_TH(N)+1,JEND_TH(N)
        DO K=0,NZM
          DO I=0,NXM
            S1(I,K,J)=((TH(I,K,J,N)+TH(I,K,J-1,N))/2.d0)
     &                  *U2(I,K,J)
          END DO
        END DO
      END DO
      ...
      DO J=JSTART_TH(N),JEND_TH(N)
        DO K=0,TNKZ
          DO I=0,NKX
            CFTH(I,K,J,N)=CFTH(I,K,J,N)-(CS1(I,K,J+1)-CS1(I,K,J))/DYF(J)
          END DO
        END DO
      END DO
...
! We are done with the TH array, set to zero 
      do J=0,NY+1
        do K=0,NX+1
          do I=0,NZ+1
            TH(I,K,J,N)=0.d0
...
C Optionally, add the implicit part of the LES SGS term
        IF (LES) THEN
        DO J=J1,J2
          DO I=0,NKX
            MATL_C(I,J)=MATL_C(I,J) + TEMP1 * (
! d/dy(KAPPA_T)*d/dy(TH):
     &           ((KAPPA_T(I,K,J+1,N)-KAPPA_T(I,K,J,N))/DYF(J))
     &                   *-1.d0/(2.d0*DY(J))
! KAPPA_T*d^2/dy^2(TH):
     &          +  KAPPA_T(I,K,J,N) 
     &           /(DYF(J)*DY(J))  )

            MATD_C(I,J)=MATD_C(I,J) + TEMP1 * (
! d/dy(KAPPA_T)*d/dy(TH):
     &           ((KAPPA_T(I,K,J+1,N)-KAPPA_T(I,K,J,N))/DYF(J))
     &          * (DYF(J+1)/(2.d0*DYF(J+1)*DYF(J))
     &            +DYF(J-1)/(2.d0*DY(J)*DYF(J)))  
! KAPPA_T*d^2/dy^2(TH):
     &           -KAPPA_T(I,K,J,N)
     &           *(-1.d0/(DYF(J)*DY(J+1)) -1.d0/(DYF(J)*DY(J))) )

            MATU_C(I,J)=MATU_C(I,J) + TEMP1 * (
! d/dy(KAPP_T)*d/dy(TH):
     &           ((KAPPA_T(I,K,J+1,N)-KAPPA_T(I,K,J,N))/DYF(J)) 
     &              *-1.d0/(2.d0*DY(J+1))  
! KAPPA_T*d^2/dy^2(TH):
...
! and backward sweeps for efficiency
          IF (USE_MPI) THEN
             CALL APPLY_BC_TH_MPI_C(MATL_C,MATD_C,MATU_C,VEC_C,K,N)
! If we are using MPI, split the implicit solve into foward and
! backward sweeps for efficiency
             CALL THOMAS_FORWARD_COMPLEX_MPI
     &                            (MATL_C,MATD_C,MATU_C,VEC_C,NY,NKX)
             CALL THOMAS_BACKWARD_COMPLEX_MPI
     &                            (MATL_C,MATD_C,MATU_C,VEC_C,NY,NKX)
          ELSE
...
! END do k
      END DO 
       ...
       write(*,*) 'post solve TH: ',CTH(10,10,10,1)

! End do number of passive scalars
        END DO

! We are done with the Ui arrays, set to zero 
      do J=0,NY+1
        do K=0,NX+1
          do I=0,NZ+1
            U1(I,K,J)=0.d0
...
! First, apply the boundary conditions
          CALL APPLY_BC_U2_MPI_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
...
! End do k
      END DO 
       ...
       write(*,*) 'post solve U2: ',CU2(10,10,10)
...
! NU_T*d^2/dxdy(U2):
     &           +NU_T(I,K,J)
     &           *(CIKX(I)*(CU2(I,K,J+1)-CU2(I,K,J))/DYF(J)) )
...
! NU_T*d^2/dy^2(U1):
     ...
     &           /(DY(J+1)*DYF(J))  )
        ...
        IF (USE_MPI) THEN
! First, apply the boundary conditions
          CALL APPLY_BC_U1_MPI_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
! If we are using MPI, split the implicit solve into forward and
! backward sweeps for efficiency
          CALL THOMAS_FORWARD_COMPLEX_MPI
     &                           (MATL_C,MATD_C,MATU_C,VEC_C,NY,NKX)
...
C Set the boundary conditions for U1
          ...
          if (k.eq.10) then
            do j=0,NY+1
              write(700,*) 'J: ',J,MATL_C(10,J),MATD_C(10,J)
     &            ,MATU_C(10,J)
     &            ,VEC_C(10,J)
            end do
          end if
...
! End do k
      END DO
       ...
       write(*,*) 'post solve U2: ',CU1(10,10,10)
...
! NU_T*d^2/dy^2(U1):
     ...
     &           /(DY(J+1)*DYF(J))  )
        ...
        IF (USE_MPI) THEN
! First, apply the boundary conditions
          CALL APPLY_BC_U3_MPI_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
! If we are using MPI, split the implicit solve into forward and
! backward sweeps for efficiency
          CALL THOMAS_FORWARD_COMPLEX_MPI
     &           (MATL_C,MATD_C,MATU_C,VEC_C,NY,NKX)
...
C Define grid spacing
         ...
         DO J=1,NY
           DYF(J)=(GY(J+1)-GY(J))
         END DO
C Set grid spacing for ghost cells
         DYF(0)=DYF(1)
         DY(0)=DY(1)
         DY(NY+1)=DY(NY)
...
! Calculate the viscous PV fluxes
     ...
     &                                /DYF(j)
        ...
        IF (MPI_NUM.eq.7) then
          write(*,*) 'Movie TH,: ',TH(0,0,NY,1)
        END IF
     ...
     &        ,form='formatted')
        write(*,*) 'TH(10,10,NY): ',TH(10,10,NY/2,1)
        do I=0,NXM
        do K=0,NZM
          write(89,*) TH(I,K,NY/2,1)+DRHODX*GX(I)
        end do
        end do
...
! This file will contain a time history over a plane
     ...
     &        ,form='formatted')
        do I=0,NXM
        do K=0,NZM
          write(85,*) U3(I,K,NY)+RI_TAU(1)*DRHODX/I_RO_TAU
        end do
        end do
...
C----*|--.---------.---------.---------.---------.---------.---------.-|------
      SUBROUTINE APPLY_BC_1_LOWER_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
C----*|--.---------.---------.---------.---------.---------.---------.-|-----
      INCLUDE 'header'
      INTEGER I,K
...
C----*|--.---------.---------.---------.---------.---------.---------.-|----
      SUBROUTINE APPLY_BC_1_UPPER_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      INCLUDE 'header'
      INTEGER I,K
...
C----*|--.---------.---------.---------.---------.---------.---------.-|---
      SUBROUTINE APPLY_BC_2_LOWER_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      INCLUDE 'header'
      INTEGER I,K
...
C Dirichlet
        ...
        IF (K.EQ.0) THEN
          ...
          MATD_C(0,1)=1.d0
          MATU_C(0,1)=0.d0                   
          VEC_C(0,1)=V_BC_YMIN_C1 

          MATL_C(0,2)=0.d0
          MATD_C(0,2)=1.d0
          MATU_C(0,2)=0.d0
          VEC_C(0,2)=V_BC_YMIN_C1
        END IF
...
C The following is only a placeholder, this row is used for U1 and U3 only
      DO I=0,NKX
        MATL_C(I,0) = 0.d0
        MATD_C(I,0) = 1.d0
        MATU_C(I,0) = 0.d0
...
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      SUBROUTINE APPLY_BC_2_UPPER_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      INCLUDE 'header'
      INTEGER I,K
C Top wall
...
C Dirichlet
        ...
        IF (K.eq.0) THEN
          ...
          MATD_C(0,NY+1)=1.d0
          MATU_C(0,NY+1)=0.d0
          VEC_C(0,NY+1)=V_BC_YMAX_C1

          MATL_C(0,NY)=0.d0
          MATD_C(0,NY)=1.d0
          MATU_C(0,NY)=0.d0
          VEC_C(0,NY)=V_BC_YMAX_C1
        END IF
...
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      SUBROUTINE APPLY_BC_3_LOWER_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      INCLUDE 'header'
      INTEGER I,K
...
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      SUBROUTINE APPLY_BC_3_UPPER_C(MATL_C,MATD_C,MATU_C,VEC_C,K)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      INCLUDE 'header'
      INTEGER I,K
...
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      subroutine APPLY_BC_TH_LOWER_C(MATL_C,MATD_C,MATU_C,VEC_C,K,N)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      include 'header'
      integer i,N,K
! Bottom Wall:
...
! Dirichlet
        ...
        IF (K.eq.0) THEN
          ...
          MATD_C(0,1)=1.d0
          MATU_C(0,1)=0.d0
          VEC_C(0,1)=TH_BC_YMIN_C1(N)

          MATL_C(0,2)=0.d0
          MATD_C(0,2)=1.d0
          MATU_C(0,2)=0.d0
          VEC_C(0,2)=TH_BC_YMIN_C1(N)
        END IF
...
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      subroutine APPLY_BC_TH_UPPER_C(MATL_C,MATD_C,MATU_C,VEC_C,K,N)
C----*|--.---------.---------.---------.---------.---------.---------.-|--
      include 'header'
      integer i,N,K
! Top wall
...
! Dirichlet
        ...
        IF (K.eq.0) THEN
          ...
          MATD_C(0,NY)=1.d0
          MATU_C(0,NY)=0.d0
          VEC_C(0,NY)=TH_BC_YMAX_C1(N)

          MATL_C(0,NY-1)=0.d0
          MATD_C(0,NY-1)=1.d0
          MATU_C(0,NY-1)=0.d0
          VEC_C(0,NY-1)=TH_BC_YMAX_C1(N)
        END IF
      ...
      do j=JSTART,JEND
        do k=0,NZM
          do i=0,NXM
            ...
            dt_z=abs(cfl*dz(k)/abs(U3(i,k,j)))
