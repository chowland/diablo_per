...
C******************************************************************************|
C--------> The following routines for the MPI periodic.f <---------|
C******************************************************************************|

C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      SUBROUTINE FFT_Y_TO_FOURIER(U,CU,IMIN,IMAX,KMIN,KMAX)
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
C This routine transforms (in 1 direction) planes JMIN-JMAX to Fourier space.
C----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      INCLUDE 'header'
      INTEGER IMIN, IMAX, KMIN, KMAX, I, J, K
      REAL*8 U (0:NX+1,0:NZ+1,0:NY+1)
      REAL*8 CU(0:NX+1,0:NZ+1,0:NY/2)

C Looping over the planes of interest, perform a real -> real
C transform in place in the big storage array, scaling appropriately.

      DO J=JMIN,JMAX
       CALL RFFTWND_F77_REAL_TO_COMPLEX(FFTW_X_TO_F_PLAN,(KMAX-KMIN+1),
     *    U(0,KMIN,J), 1, NX_P+2, CU(0,KMIN,J), 1, NX_P/2+1)
        DO K=KMIN,KMAX
          DO I=0,NKX
            CU(I,K,J)=CU(I,K,J)/RNX
          END DO
