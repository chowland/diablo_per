...
! This subroutine initializes all mpi variables
      ...
      real*8 SENDBUF(0:NX_S,0:TNKZ_S,0:TNKY_S)
      real*8 RECVBUF(0:NX_S,0:TNKZ_S,0:TNKY_S)
      ...
      sendcount=(NX_S+1)*(TNKZ_S+1)*(TNKY_S+1)
      recvcount=sendcount
...
! Get a block from the source array to be sent to another process
        DO I=0,NX_S
          DO K=0,TNKZ_S
            DO J=0,TNKY_S
              SENDBUF(I,K,J)=A_Z(I,K+(TNKZ_S+1)*BLOCK,J)
            END DO
          END DO
        END DO
...
! Place the newly recieved data into a temporary storage array
        DO I=0,NX_S
          DO K=0,TNKZ_S
            DO J=0,TNKY_S
              IF (K+(TNKZ_S+1)*BLOCK.le.2*(NZ-1)/3) THEN
! Make sure that we don't write beyond the end of the array
                B(I,K+(TNKZ_S+1)*BLOCK,J)=RECVBUF(I,K,J)
              END IF
            END DO
          END DO
        END DO
...
! Here block=diag, we just need to copy locally from A->B
          ...
          DO I=0,NX_S
            DO K=0,TNKZ_S
              DO J=0,TNKY_S
              IF (K+(TNKZ_S+1)*BLOCK.le.2*(NZ-1)/3) THEN
                  B(I,K+(TNKZ_S+1)*BLOCK,J)=sendbuf(I,K,J)
              END IF
              END DO
            END DO
          END DO
...
! This subroutine initializes all mpi variables
      ...
      real*8 SENDBUF(0:NX_S,0:NZ_S,0:TNKY_S)
      real*8 RECVBUF(0:NX_S,0:NZ_S,0:TNKY_S)
      ...
      sendcount=(NX_S+1)*(NZ_S+1)*(TNKY_S+1)
      recvcount=sendcount
...
! Get a block from the source array to be sent to another process
        DO I=0,NX_S
          DO K=0,NZ_S
            DO J=0,TNKY_S
              SENDBUF(I,K,J)=A_Y(I,K,J+(TNKY_S+1)*BLOCK)
            END DO
          END DO
        END DO
...
! Here block=diag, we just need to copy locally from A->B
          ...
          DO I=0,NX_S
            DO K=0,NZ_S
              DO J=0,TNKY_S
              IF (K+(NZ_S+1)*BLOCK.le.2*(NZ-1)/3) THEN
...
! Place the newly recieved data into a temporary storage array
        DO I=0,NX_S
          DO K=0,NZ_S
            DO J=0,TNKY_S
              IF (J+(TNKY_S+1)*BLOCK.le.2*(NY-1)/3) THEN
! Make sure that we don't write beyond the end of the array
                B(I,K,J+(TNKY_S+1)*BLOCK)=RECVBUF(I,K,J)
              END IF
            END DO
          END DO
        END DO
...
! Here block=diag, we just need to copy locally from A->B
          ...
          DO I=0,NX_S
            DO K=0,NZ_S
              DO J=0,TNKY_S
              IF (J+(TNKY_S+1)*BLOCK.le.2*(NY-1)/3) THEN
                B(I,K,J+(TNKY_S+1)*BLOCK)=sendbuf(I,K,J)
              END IF
              END DO
            END DO
          END DO
